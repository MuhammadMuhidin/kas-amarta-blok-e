...
export async function checkApprovalStatus({ requestNo, requesterKey }) {
  const supabase = getSupabaseAdmin();
  const no = clean(requestNo).toUpperCase();
  const key = clean(requesterKey);

  if (!no) throw new Error("Nomor pengajuan wajib diisi");
  if (!key) throw new Error("Nomor rumah atau WhatsApp wajib diisi");
  ...
  const allowed = [request.requester_house, request.requester_phone]
    .map(clean)
    .some((value) => value && value === key);

  if (!allowed) throw new Error("Data verifikasi tidak sesuai");
  ...
  return { ok: true, request, actions: actions || [] };
}

export async function actOnApprovalRequest({ req, accessRole = "admin", id, action, note }) {
  const supabase = getSupabaseAdmin();
  const role = resolveAdminAccessRole(accessRole);
  ...
  if (selectedAction === "reject") {
    ...
    await recordAdminActivity(req, {
      ...
      severity: "warning",
      message: `Reject approval ${request.request_no}`,
      metadata: { access_role: role, request_no: request.request_no },
    });
  ...
  const currentStep = getStep(master?.flow_schema || [], request.current_step);
  const nextStep = getNextStep(master?.flow_schema || [], request.current_step);
  const nextStatus = nextStep ? (nextStep.action === "validate_payment" ? "waiting_payment_validation" : "waiting_approval") : "completed";
  const now = new Date().toISOString();
  const updatePayload = {
    status: nextStatus,
  ...
  if (currentStep?.action === "validate_payment" || selectedAction === "validate_payment") {
  ...
  if (error) throw new Error(error.message || "Gagal approve pengajuan");
  ...
  await supabase.from(APPROVAL_ACTIONS_TABLE).insert({
    ...
    role,
    actor: role,
    action: currentStep?.action === "validate_payment" ? "validate_payment" : "approve",
    note: clean(note),
  });
  ...
  await recordAdminActivity(req, {
    ...
    module: "approval-center",
    severity: "success",
    message: `${nextStatus === "completed" ? "Complete" : "Approve"} approval ${request.request_no}`,
    metadata: { access_role: role, request_no: request.request_no, next_status: nextStatus },
  });
