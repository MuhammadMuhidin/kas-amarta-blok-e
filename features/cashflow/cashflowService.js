...
import {
  insertCashflow,
  listCashflows,
} from "@/features/cashflow/cashflowRepository";
...
export async function listCashflowRecords({ supabase, searchParams }) {
  const data = await listCashflows(supabase);
...
export async function createDirectCashflow({ supabase, req, body, isMultipart }) {
  ...
  if (!["income", "expense"].includes(type)) {
    return {
      status: 400,
      body: { error: "Type must be income or expense" },
    };
  }
  ...
  if (!Number.isFinite(amount) || amount <= 0 || !rawNote) {
    return {
      status: 400,
      body: { error: "Amount and note are required" },
    };
  }
  ...
  return {
    status: 200,
    body: { success: true, receipt_url: withMediaReceiptUrl({ receipt_url: receiptUrl }).receipt_url },
  };
}
