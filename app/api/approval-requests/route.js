...
import { queueApprovalCreatedNotification } from "@/lib/notificationQueue";
...
function maskResidentHouse(value) {
  ...
  if (!raw) return "-";
  const normalized = raw.replace(/^\s*(?:BLOK|BLK)\s+/i, "").replace(/\b(?:NOMOR|NO|UNIT)\b\.?/gi, "-").replace(/[\/#]/g, "-").replace(/\s*-\s*/g, "-").replace(/\s+/g, " ").trim();
...
function requestReason(row = {}) { return clean(row.form_data?.reason || row.form_data?.alasan || row.reason); }
function publicRequest(row = {}) {
  return { id: row.id, request_no: row.request_no, master_name: row.master_name, master_code: row.master_code, status: row.status, current_step: row.current_step, current_approver_role: row.current_approver_role, amount: row.amount, payment_status: row.payment_status, submitted_at: row.submitted_at, updated_at: row.updated_at, completed_at: row.completed_at, requester_name: maskResidentName(row.requester_name), requester_house: maskResidentHouse(row.requester_house), reason: requestReason(row) };
}
function publicAction(row = {}) {
  const resident = clean(row.role).toLowerCase() === "warga" || clean(row.action).toLowerCase() === "submit";
  return { id: row.id, step: row.step, role: row.role, actor: resident ? maskResidentName(row.actor) : row.actor, action: row.action, note: row.note, created_at: row.created_at };
}
function publicFlowStep(row = {}, index = 0) { return { step: number(row.step) || index + 1, role: clean(row.role).toLowerCase(), label: clean(row.label), action: clean(row.action) || "approve" }; }
function normalizeFlow(value = []) { return [...(Array.isArray(value) ? value : [])].map(publicFlowStep).filter((step) => step.role).sort((a, b) => a.step - b.step); }
...
async function checkStatusByRequestNo(requestNo) {
  ...
  const { data: request, error } = await supabase.from(APPROVAL_REQUESTS_TABLE).select("*").eq("request_no", no).maybeSingle();
  ...
  const [{ data: actions, error: actionsError }, flowSchema] = await Promise.all([
    supabase.from(APPROVAL_ACTIONS_TABLE).select("*").eq("request_id", request.id).order("created_at", { ascending: true }),
  ...
  return { ok: true, request: publicRequest(request), flow_schema: flowSchema, actions: (actions || []).map(publicAction) };
}

function safeFileName(value) { const raw = clean(value).normalize("NFKD").replace(/[^a-zA-Z0-9._-]+/g, "-").replace(/-+/g, "-"); return raw.replace(/^[-.]+|[-.]+$/g, "").slice(0, 100) || "attachment"; }
function acceptedMimeTypes(field = {}) { return clean(field.accept).split(",").map((item) => clean(item).toLowerCase()).filter(Boolean); }
function mimeAccepted(fileType, accepted = []) { if (!accepted.length) return true; const type = clean(fileType).toLowerCase(); return accepted.some((rule) => rule.endsWith("/*") ? type.startsWith(rule.slice(0, -1)) : type === rule); }
...
async function ensureAttachmentBucket(supabase) {
  ...
  const { error: createError } = await supabase.storage.createBucket(ATTACHMENT_BUCKET, { public: false, fileSizeLimit: MAX_TOTAL_UPLOAD_BYTES, allowedMimeTypes: ["application/pdf", "image/jpeg", "image/png", "image/webp"] });
  if (createError && !clean(createError.message).toLowerCase().includes("already exists")) throw new Error(createError.message || "Gagal menyiapkan penyimpanan lampiran");
}

async function parseSubmission(req) {
  const contentType = clean(req.headers.get("content-type")).toLowerCase();
  if (!contentType.includes("multipart/form-data")) return { payload: await req.json(), multipart: null };
  ...
  let payload;
  try { payload = JSON.parse(String(rawPayload)); } catch { throw new Error("Payload pengajuan tidak valid"); }
...
async function uploadSubmissionAttachments({ payload, multipart }) {
  ...
  const files = uploadFields.map((field) => ({ field, file: multipart.get(`attachment:${field.key}`) })).filter(({ file }) => file && typeof file.arrayBuffer === "function" && file.size > 0);
  ...
  try {
    for (const { field, file } of files) {
      ...
      const { error } = await supabase.storage.from(ATTACHMENT_BUCKET).upload(path, bytes, { contentType: file.type || "application/octet-stream", cacheControl: "3600", upsert: false });
      if (error) throw new Error(error.message || `Gagal mengunggah ${field.label}`);
      uploadedPaths.push(path);
      nextFormData[field.key] = { kind: "attachment", bucket: ATTACHMENT_BUCKET, path, name: safeFileName(file.name), original_name: clean(file.name), type: clean(file.type) || "application/octet-stream", size: Number(file.size || 0) };
    }
...
export async function GET(req) {
  try {
    ...
    if (requestNo) return NextResponse.json(await checkStatusByRequestNo(requestNo), { headers: { "Cache-Control": "private, no-store, max-age=0" } });
...
export async function POST(req) {
  ...
  try {
    ...
    const activeFlow = normalizeFlow(result.request?.form_data?.__system?.flow_schema_snapshot || []);
    const activeAction = activeFlow.find((step) => step.step === number(result.request?.current_step))?.action || "approve";
    const telegramQueue = await queueApprovalCreatedNotification({ request: result.request, activeAction });
    return NextResponse.json({ ...result, whatsapp_sent: requesterWhatsapp.sent, whatsapp_status: requesterWhatsapp, first_approver_whatsapp_sent: firstApproverWhatsapp.sent, first_approver_whatsapp_status: firstApproverWhatsapp, telegram_queue: telegramQueue });
  } catch (err) {
