...
function ActionModal({ pendingAction, pin, setPin, running, contactForm, setContactForm, onCancel, onConfirm }) {
  ...
  return <div className={modalStyles.overlay} onClick={onCancel}><div className={modalStyles.box} onClick={(event) => event.stopPropagation()}><div style={modalTitleStyle}>{pendingAction.title}</div><div style={modalNoteStyle}>{pendingAction.description}</div>{pendingAction.type === "edit_contact" && <div style={formGroupStyle}><label style={inputLabelStyle}><span>WhatsApp OTP</span><input className="admin-input" value={contactForm.phone} onChange={(event) => setContactForm((prev) => ({ ...prev, phone: event.target.value }))} placeholder="628xxxxxxxxxx" /></label><label style={inputLabelStyle}><span>Telegram User ID</span><input className="admin-input" inputMode="numeric" value={contactForm.telegram_user_id} onChange={(event) => setContactForm((prev) => ({ ...prev, telegram_user_id: event.target.value.replace(/\D/g, "").slice(0, 20) }))} placeholder="123456789" /></label></div>}<label style={inputLabelStyle}><span>Admin PIN</span><input className="admin-input" type="password" value={pin} onChange={(event) => setPin(event.target.value)} placeholder="Enter admin PIN" /></label><div style={modalButtonGridStyle}><button type="button" className="admin-small-btn" disabled={running} onClick={onCancel}>Cancel</button><button type="button" className="admin-small-btn" disabled={running || !pin} onClick={onConfirm}><LoadingButtonContent loading={running} loadingText="Processing...">Confirm</LoadingButtonContent></button></div></div></div>;
}
...
export default function RoleManagementTab() {
  ...
  const [contactForm, setContactForm] = useState({ role: "", phone: "", telegram_user_id: "" });
  ...
  function openEditContact(row) {
    ...
    openAction({ type: "edit_contact", title: `Edit Contact - ${row.label}`, description: "Update WhatsApp OTP destination and Telegram User ID for this role." });
  }
  ...
  function getActionPayload() {
    if (pendingAction?.type === "edit_contact") {
      return {
        ...
        telegram_user_id: contactForm.telegram_user_id,
      };
    }
