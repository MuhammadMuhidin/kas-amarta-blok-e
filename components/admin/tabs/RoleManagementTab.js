...
function RoleOverviewCard({ rows, accessRows, onViewAll }) {
  return <SectionCard title="Role Overview" description="Current role readiness, menu access, sessions, and latest activity." action={<ActionButton onClick={onViewAll}>View All</ActionButton>}><div className="admin-monitor-grid" style={styles.roleOverviewGrid}>{rows.map((role) => { const access = accessRows.find((item) => item.role === role.role); return <div key={role.role} className="admin-status-card" style={styles.roleTile}><div style={styles.tileHeader}><div><div className="admin-status-label">{role.label}</div><div className="admin-status-value" style={styles.tileValue}>{role.menu_count}/{role.menu_total}</div><div className="admin-status-meta">visible menus</div></div><StatusBadge status={role.contact_active ? "active" : "inactive"}>{role.contact_active ? "Active" : "Inactive"}</StatusBadge></div><div style={styles.tileMetaGrid}><InfoPill label="Sessions" value={role.active_sessions} /><InfoPill label="Last Login" value={formatTime(role.last_login_at)} /><InfoPill label="Last Activity" value={formatTime(role.last_activity_at)} /><InfoPill label="Access" value={access ? `${access.allowed_count}/${access.total_count} menus` : "-"} /></div></div>; })}</div></SectionCard>;
}
