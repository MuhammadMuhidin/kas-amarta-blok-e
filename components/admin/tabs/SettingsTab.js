...
function statusText(status) {
  ...
  if (!config.bot_token_configured || !config.chat_id_configured) return "Bot Token atau Chat ID belum lengkap";
  if (!config.webhook_secret_configured) return "Webhook secret belum dikonfigurasi";
  ...
  return status.webhook.pending_update_count ? `Webhook aktif • ${status.webhook.pending_update_count} update menunggu` : "Webhook aktif dan siap menerima action";
}
...
export default function AdminSettings() {
  ...
  async function loadTelegram() { setLoadingTelegram(true); try { const res = await fetch("/api/admin/settings/telegram", { cache: "no-store" }); const data = await res.json(); if (!res.ok) throw new Error(data.error); setTelegramStatus(data); } catch (error) { setTelegramStatus({ webhook_error: error.message || "Failed to load Telegram status" }); } finally { setLoadingTelegram(false); } }
  ...
  function telegramAction(action) { requestPin(async (pin) => { setSavingTelegram(true); try { const res = await fetch("/api/admin/settings/telegram", { method: "POST", headers: { "Content-Type": "application/json", "x-csrf-token": cookie("csrf_token") }, body: JSON.stringify({ action, pin }) }); const data = await res.json(); if (!res.ok) throw new Error(data.error); setTelegramStatus(data.status || telegramStatus); notify(setPopup, "Telegram action completed"); } catch (error) { notify(setPopup, error.message || "Telegram action failed", "error"); } finally { setSavingTelegram(false); } }); }
  ...
  useEffect(() => { loadConfig(); loadAppConfig(); loadTelegram(); setTheme(savedTheme()); }, []);
  ...
  const busy = saving || savingConfig || savingMatrix || savingTelegram;
  ...
  return <div style={styles.card}>
    ...
    <section style={styles.integrationCard}>
      <div style={styles.integrationHeader}><div><h3 style={styles.rowTitle}>Telegram Integration Health</h3><p style={styles.desc}>{loadingTelegram ? "Reading Telegram status..." : statusText(telegramStatus)}</p></div><button type="button" className="admin-small-btn" disabled={loadingTelegram || busy} onClick={loadTelegram}>Refresh</button></div>
      <div style={styles.statusGrid}><Pill label="Bot Token" ready={telegramStatus?.config?.bot_token_configured} /><Pill label="Chat ID" ready={telegramStatus?.config?.chat_id_configured} /><Pill label="Webhook Secret" ready={telegramStatus?.config?.webhook_secret_configured} /><Pill label={`Authorized Users: ${telegramStatus?.config?.authorized_user_count || 0}`} ready={Number(telegramStatus?.config?.authorized_user_count || 0) > 0} /></div>
      <div style={styles.integrationActions}><button type="button" className="admin-small-btn" disabled={busy} onClick={() => telegramAction("test_direct")}>Test Direct</button><button type="button" className="admin-small-btn" disabled={busy || !config.telegramNotificationsEnabled} onClick={() => telegramAction("test_queue")}>Test Queue</button><button type="button" className="admin-small-btn" disabled={busy} onClick={() => telegramAction("register_webhook")}>Register Webhook</button><button type="button" className="admin-small-btn" disabled={busy || !telegramStatus?.webhook?.url} onClick={() => telegramAction("remove_webhook")}>Remove Webhook</button></div>
    </section>
...
function Pill({ label, ready }) { return <span style={{ ...styles.statusPill, borderColor: ready ? "#16a34a" : "#dc2626", color: ready ? "#16a34a" : "#dc2626" }}>{ready ? "✓" : "×"} {label}</span>; }
function ConfigItem({ label, description, type, value, resetKey, onSave, disabled, saving, isMobile }) {
  const [local, setLocal] = useState(value); useEffect(() => setLocal(value), [value, resetKey]); const unchanged = String(local) === String(value);
...
const styles = {
  ...
  integrationCard: { margin: "18px 0 24px", padding: 16, borderRadius: 14, border: "1px solid var(--admin-border)", background: "var(--admin-row)" }, integrationHeader: { display: "flex", justifyContent: "space-between", alignItems: "flex-start", gap: 12, flexWrap: "wrap" }, statusGrid: { display: "flex", flexWrap: "wrap", gap: 8, marginTop: 14 }, statusPill: { display: "inline-flex", border: "1px solid", borderRadius: 999, padding: "6px 10px", fontSize: 12, fontWeight: 800 }, integrationActions: { display: "flex", flexWrap: "wrap", gap: 8, marginTop: 14 },
