...
function date(value) {
  ...
  return value && !Number.isNaN(parsed.getTime())
    ? parsed.toLocaleString("id-ID", { day: "2-digit", month: "short", year: "numeric", hour: "2-digit", minute: "2-digit" })
    : "-";
}
...
function meta(status) {
  ...
  if (value === "waiting_payment_validation" || value === "waiting_approval") return ["In Progress", "ac-progress"];
...
function actionLabel(action) {
  ...
  if (["approve", "final_approval", "validate_document", "confirm_execution"].includes(value)) return "Approved";
  if (value === "submit") return "Submitted";
  return value || "Action";
}

function waitingLabel(row) {
  return roleKey(row?.status) === "waiting_payment_validation" ? "Waiting for Validation" : "Waiting Approval";
}

function waitingClass(row) {
  return roleKey(row?.status) === "waiting_payment_validation" ? "ac-step-wait" : "ac-step-approval-wait";
}
...
function Summary({ label, value, active, onClick }) {
  return (
    <button type="button" className={active ? "admin-summary-card admin-summary-card-active" : "admin-summary-card"} style={{ cursor: "pointer", border: "1px solid var(--admin-border)" }} onClick={onClick}>
      <div className="admin-status-label">{label}</div>
      <div className="admin-status-value" style={{ marginBottom: 0 }}>{value}</div>
    </button>
  );
}

function Steps({ row }) {
  const actions = (row.approval_actions || []).filter((action) => !["submit", "reject"].includes(roleKey(action.action)));
  const approved = [...new Map(actions.map((action) => [`${action.step}-${action.role}`, action])).values()];
  return (
    <div className="ac-steps">
      {approved.map((action) => <div key={action.id || `${action.step}-${action.role}`} className="ac-step ac-step-ok"><small>{action.role}</small><span>Approved</span></div>)}
      {row.current_approver_role && !done(row.status) ? <div className={`ac-step ${waitingClass(row)}`}><small>{row.current_approver_role}</small><span>{waitingLabel(row)}</span></div> : null}
    </div>
  );
}

function Notes({ row }) {
  const notes = (row.approval_actions || []).filter((action) => String(action.note || "").trim());
  if (!notes.length) return null;
  return (
    <div className="ac-notes">
      <span className="ac-label">Approval Notes</span>
      {notes.map((action) => <div key={action.id || `${action.role}-${action.created_at}`} className="ac-note-row"><small>{action.role || "-"} · {actionLabel(action)} · {date(action.created_at)}</small><div className="ac-note-text">{action.note}</div></div>)}
    </div>
  );
}

function submissionLabel(row, key) {
  return (row.fields_schema_snapshot || []).find((field) => field.key === key)?.label || key.replace(/_/g, " ");
}
...
function AttachmentValue({ value }) {
  ...
  return (
    <div className="ac-attachment">
      {isImage && value.signed_url ? <img src={value.signed_url} alt={value.original_name || value.name || "Lampiran"} loading="lazy" /> : <span className="ac-file-icon">{isImage ? "🖼️" : "📎"}</span>}
      <div><strong>{value.original_name || value.name || "Lampiran"}</strong><small>{[value.type, fileSize(value.size)].filter(Boolean).join(" · ")}</small>{value.preview_error ? <em>Preview tidak tersedia</em> : null}</div>
      {value.signed_url ? <a href={value.signed_url} target="_blank" rel="noreferrer">Buka</a> : null}
    </div>
  );
}
      ...
      <div className="ac-submission-grid">
        {entries.map(([key, value]) => (
          <div className="ac-submission-field" key={key}>
            ...
            {value && typeof value === "object" && value.kind === "attachment" ? <AttachmentValue value={value} /> : <div className="ac-submission-value">{displayValue(value)}</div>}
          </div>
        ))}
      </div>
    </details>
...
function Cards({ rows, inboxIds, runningId, onAction }) {
  ...
  return (
    <div className="ac-list">
      {rows.map((row) => {
        const action = row.status === "waiting_payment_validation" ? "validate_payment" : "approve";
        ...
        return (
          <article className="ac-card" key={row.id}>
            <div className="ac-head"><div><div className="ac-no">{row.request_no}</div><div className="ac-type">{row.master_name || "-"}</div></div><Status status={row.status} /></div>
            <Steps row={row} />
            <Notes row={row} />
            ...
            {canAct ? <div className="ac-actions"><button className="admin-small-btn" disabled={Boolean(runningId)} onClick={() => onAction(row, action)}>{runningId === `${row.id}-${action}` ? "Processing..." : label}</button><button className="admin-small-btn ac-reject" disabled={Boolean(runningId)} onClick={() => onAction(row, "reject")}>{runningId === `${row.id}-reject` ? "Processing..." : "Reject"}</button></div> : null}
          </article>
        );
      })}
    </div>
...
export default function ApprovalCenterTab() {
  const [data, setData] = useState({ summary: {}, inbox: [], requests: [], pagination: { offset: 0, limit: FETCH_LIMIT, next_offset: 0, total: 0, has_more: false } });
  ...
  const all = useMemo(() => {
    const map = new Map();
    [...(data.inbox || []), ...(data.requests || [])].forEach((row) => { if (row?.id) map.set(row.id, row); });
    return [...map.values()];
  }, [data]);
  const safeInbox = useMemo(() => all.filter((row) => actionable(row, accessRole)), [all, accessRole]);
  const inboxIds = useMemo(() => new Set(safeInbox.map((row) => row.id)), [safeInbox]);
  const summary = useMemo(() => data.summary || { inbox: 0, processing: 0, completed: 0, rejected: 0 }, [data.summary]);
  ...
  const chooseFilter = (key) => setFilter((previous) => previous === key ? "all" : key);
  const pageUrl = useCallback((offset = 0) => {
    const params = new URLSearchParams({ offset: String(offset), limit: String(FETCH_LIMIT), filter, search: debouncedSearch });
  ...
  const load = useCallback(async ({ reset = false } = {}) => {
    try {
      ...
      show(error.message || "Failed to load Approval Center", "error");
    } finally {
      setLoading(false);
    }
  }, [pageUrl]);
  ...
  async function loadMore() {
    ...
    try {
      ...
      setData((previous) => ({ ...payload, requests: mergeRows(previous.requests, payload.requests), inbox: mergeRows(previous.inbox, payload.inbox), summary: payload.summary || previous.summary, pagination: payload.pagination || previous.pagination }));
    } catch (error) {
      show(error.message || "Failed to load more approval requests", "error");
    } finally {
  ...
  async function confirm() {
    ...
    if (action === "reject" && !cleanedNote) return show("Rejection note is required", "error");
    try {
      ...
      show(error.message || "Failed to process approval request", "error");
      if (error.status === 409) await load({ reset: false });
    } finally {
      setRunningId("");
    }
  }
  ...
  useEffect(() => { load({ reset: false }); }, [load]);
  ...
  return (
    <>
      ...
      <AdminConfirmModal open={Boolean(pending)} title={reject ? "Reject Request" : "Confirm Approval"} description={row ? `${row.request_no} - ${row.master_name || "Approval Request"}` : ""} confirmText={reject ? "Reject" : "Process"} cancelText="Cancel" loading={Boolean(runningId)} loadingText="Processing..." onCancel={close} onConfirm={confirm}>
        <div><div><strong>Status</strong><p>{row ? meta(row.status)[0] : "-"}</p></div><div><strong>Requester</strong><p>{row?.requester_name || "-"} · {row?.requester_house || "-"}</p></div><div><strong>{reject ? "Rejection Note" : "Approval Note"}</strong><textarea className="ac-note" value={note} onChange={(event) => setNote(event.target.value)} placeholder={reject ? "Required rejection reason" : "Optional"} /></div></div>
      </AdminConfirmModal>
      <div className="admin-card" style={{ height: "auto", overflow: "visible" }}>
        <div className="activity-header" style={{ marginBottom: 14 }}><div><div className="activity-kicker">Approval Workflow</div><h3 className="activity-title ac-title">Approval Center</h3><p className="activity-subtitle">Click a summary card to filter. Form details and private attachments are available inside each request.</p></div></div>
        <div className="admin-summary-cards ac-summary" style={{ marginBottom: 0 }}><Summary label="Need Action" value={summary.inbox || 0} active={filter === "inbox"} onClick={() => chooseFilter("inbox")} /><Summary label="In Progress" value={summary.processing || 0} active={filter === "processing"} onClick={() => chooseFilter("processing")} /><Summary label="Full Approved" value={summary.completed || 0} active={filter === "completed"} onClick={() => chooseFilter("completed")} /><Summary label="Rejected" value={summary.rejected || 0} active={filter === "rejected"} onClick={() => chooseFilter("rejected")} /></div>
        <div className="ac-toolbar"><input className="admin-search-input" value={search} onChange={(event) => setSearch(event.target.value)} placeholder="Search request no, type, requester, house, approver, note..." /></div>
        <section className="admin-status-card ac-section" style={{ margin: 0 }}>{initialLoading ? <AdminDataSkeleton showSummary={false} rows={5} /> : <Cards rows={shown} inboxIds={inboxIds} runningId={runningId} onAction={open} />}<div ref={loader} className="ac-loader">{initialLoading ? "" : loadingMore ? "Loading more..." : more ? "Scroll to load more" : ""}</div></section>
      </div>
    </>
  );
}
...
@media(max-width:640px){.ac-title{font-size:28px!important}.ac-summary{grid-template-columns:repeat(2,minmax(0,1fr))!important;gap:8px!important}.ac-section{padding:14px!important;border-radius:16px!important}.ac-status{text-align:right;font-size:12px}.ac-no{font-size:15px}.ac-submission-grid{grid-template-columns:1fr}.ac-attachment{grid-template-columns:auto minmax(0,1fr)}.ac-attachment a{grid-column:1/-1;text-align:center}}
`;
