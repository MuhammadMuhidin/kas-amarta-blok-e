...
async function readCredential(role) {
  ...
  const { data, error } = await getSupabaseAdmin()
    ...
    .select("role,password_hash,pin_hash,credential_version,updated_at")
...
export async function getAdminRoleCredentialStatus(role) {
  const credential = await readCredential(role);

  return {
    password_configured: Boolean(credential?.password_hash || process.env.ADMIN_PASSWORD),
    pin_configured: Boolean(credential?.pin_hash || process.env.ADMIN_PIN),
    credential_version: Number(credential?.credential_version || 0),
