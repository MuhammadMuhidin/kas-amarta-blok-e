...
export async function getTelegramRuntimeConfig() {
  ...
  return {
    ...
    threadId: clean(process.env.TELEGRAM_MESSAGE_THREAD_ID),
  };
}

export async function getTelegramAppBaseUrl() {
  const configured = await getIntegrationConfigString("APP_URL");
  const value = clean(
    configured
...
export async function telegramApi(method, payload = {}) {
  const { token } = await getTelegramRuntimeConfig();
...
export async function editTelegramCaption({ chatId, messageId, caption, replyMarkup }) {
...
export async function registerTelegramWebhook() {
  ...
  if (!baseUrl) throw new Error("APP_URL belum dikonfigurasi");
...
export async function isTelegramChatAllowed(chatId) {
  const configured = (await getTelegramRuntimeConfig()).chatId;
