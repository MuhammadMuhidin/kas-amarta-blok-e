...
async function insertActivity(payload) {
  try {
    ...
    console.error("Failed record admin activity (network):", err.message);
  }
}
...
export async function recordAdminActivity(req, activity = {}) {
  ...
  const ip = clean(activity.ip) || getIp(req);
  const deviceName = clean(activity.device_name) || getDeviceName(userAgent);

  const location =
    clean(activity.location) ||
