...
import { getStoredIntegrationConfigValue } from "@/lib/integrationConfig";
...
async function readStoredIntegrationEmailSetting() {
  const value = await getStoredIntegrationConfigValue("EMAIL_NOTIFICATIONS_ENABLED", null);
  if (value === undefined || value === null || value === "") return null;
  return parseBoolean(value, true);
}

function readEnvironmentEmailSetting() {
  const value = process.env.EMAIL_NOTIFICATIONS_ENABLED || process.env.ALERT_EMAIL_ENABLED;
  if (value === undefined || value === null || value === "") return null;
  return parseBoolean(value, true);
}

export async function isEmailNotificationsEnabled() {
  const integrationValue = await readStoredIntegrationEmailSetting();
  ...
  const environmentValue = readEnvironmentEmailSetting();
  return environmentValue === null ? true : environmentValue;
}
...
export async function getAppConfig() {
  ...
  const integrationEmailSetting = await readStoredIntegrationEmailSetting();
  const environmentEmailSetting = readEnvironmentEmailSetting();
  const email_notifications_enabled = integrationEmailSetting !== null
    ? integrationEmailSetting
    : config.email_notifications_enabled !== undefined
      && config.email_notifications_enabled !== null
      && config.email_notifications_enabled !== ""
      ? parseBoolean(config.email_notifications_enabled, true)
      : environmentEmailSetting === null
        ? true
        : environmentEmailSetting;
