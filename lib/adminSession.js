...
async function findCurrentSession(req) {
  ...
  const { data, error } = await supabase
    ...
    .select("id,expires_at,access_role")
  ...
  return {
    ...
    access_role: resolveAdminAccessRole(data.access_role),
  };
}
...
export async function getCurrentAdminSession(req) {
  return findCurrentSession(req);
}

export async function validateAdminSession(req) {
  return Boolean(await findCurrentSession(req));
}
