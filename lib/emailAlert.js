...
const appBaseUrl=process.env.NEXT_PUBLIC_APP_URL||process.env.APP_URL||process.env.VERCEL_PROJECT_PRODUCTION_URL||"";
...
function attachmentPath(value){
  ...
  try{
    ...
    if(key&&clean(r2PublicUrl))return `${clean(r2PublicUrl).replace(/\/$/,"")}/${key.replace(/^\//,"")}`;
  }catch{}
  if(path.startsWith("/")&&clean(appBaseUrl))return `${clean(appBaseUrl).replace(/\/$/,"")}${path}`;
  return "";
}
function filesOf(value){return Array.isArray(value)?value.map((item)=>({path:attachmentPath(item?.path),filename:clean(item?.filename)||"lampiran"})).filter((item)=>item.path):[];}
