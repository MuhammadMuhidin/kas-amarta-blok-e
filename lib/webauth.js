...
export function getWebAuthConfig() {
  const rpName = process.env.WEBAUTH_RP_NAME || "Amarta Residence";
...
export async function getWebAuthRpName() {
  return getIntegrationConfigString(
    "WEBAUTH_RP_NAME",
    process.env.WEBAUTH_RP_NAME || "Amarta Residence",
  );
}
...
export async function getAuthConfigs() {
  ...
  const telegramNotificationsEnabled = configs.TELEGRAM_NOTIFICATIONS_ENABLED === "true";
  const telegramActionsConfigured = configs.TELEGRAM_ACTIONS_ENABLED === "true";
  ...
  return {
    ...
    telegramActionsEnabled: telegramNotificationsEnabled && telegramActionsConfigured,
  };
}
