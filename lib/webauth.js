...
export async function isWhatsAppServicesEnabled() {
  const { data, error } = await supabase
    .from(CONFIG_WEBAUTH_TABLE)
    .select("value")
    .eq("key", "WA_SERVICES_ENABLED")
    .maybeSingle();

  if (error) {
    throw new Error("Config WA_SERVICES_ENABLED tidak dapat dibaca");
  }
  ...
  if (data.value !== "true" && data.value !== "false") {
    throw new Error("Config WA_SERVICES_ENABLED tidak valid");
  }
...
export async function assertWhatsAppServicesEnabled() {
  ...
  if (!enabled) {
    throw new Error("WhatsApp services sedang dinonaktifkan dari Settings Auth.");
  }
}
