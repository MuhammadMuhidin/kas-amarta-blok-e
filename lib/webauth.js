...
export async function saveCredential({ credentialId, publicKey, counter }) {
  ...
  if (disableOld.error) {
    throw new Error("Gagal menonaktifkan credential lama");
  }

  const { error } = await supabase.from("data_webauth").insert({
...
export async function getAuthConfigs() {
  const { data, error } = await supabase
    .from("config_webauth")
    .select("key,value")
    .in("key", ["WEB_AUTH_ENABLED", "PIN_ENABLED"]);
...
export async function updateAuthConfig(key, value) {
  const allowedKeys = ["WEB_AUTH_ENABLED", "PIN_ENABLED"];
  ...
  if (value !== "true" && value !== "false") {
    throw new Error("Config value tidak valid");
  }
