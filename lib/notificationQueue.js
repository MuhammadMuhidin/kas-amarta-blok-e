...
export async function queueApprovalDecisionNotification({ request, action, actorRole, actorName, note, source = "web" }) {
  ...
  const rejected = clean(action).toLowerCase() === "reject" || clean(request.status).toLowerCase() === "rejected";
  ...
  const text = [
    ...
    `<b>Status:</b> ${escapeHtml(statusLabel(request.status))}`,
    request.current_approver_role ? `<b>Tahap Berikutnya:</b> ${escapeHtml(roleLabel(request.current_approver_role))}` : "",
...
export async function queuePaymentProofSubmittedNotification({ proof, totalAmount }) {
  ...
  const buttons = markup(
    state.actions ? [callback("Setujui Bukti", `pa:${proof.id}`), callback("Tolak Bukti", `pr:${proof.id}`)] : [],
  );
...
export async function queuePaymentProofDecisionNotification({ proof, action, actorName, actorRole, reason, source = "web" }) {
  ...
  const rejected = clean(action).toLowerCase() === "reject" || clean(proof.status).toLowerCase() === "rejected";
