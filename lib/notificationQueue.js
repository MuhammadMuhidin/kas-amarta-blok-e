...
export async function queuePengaduanNotification({ pengaduan, photoUrl }) {
  if (!pengaduan?.id) return { queued: false, reason: "missing_pengaduan" };
  const mediaUrl = await absoluteMediaUrl(photoUrl);
  const text = [
    "<b>Pengaduan Baru</b>",
    "",
    `<b>Nama:</b> ${escapeHtml(pengaduan.nama || "-")}`,
    `<b>Rumah:</b> ${escapeHtml(pengaduan.rumah || "-")}`,
    `<b>Kritik & Saran:</b> ${escapeHtml((pengaduan.kritik || "").slice(0, 500))}`,
    photoUrl ? "<b>Lampiran:</b> Ada" : "<b>Lampiran:</b> Tidak ada",
    `<b>Dikirim:</b> ${escapeHtml(time(pengaduan.created_at))}`,
  ].join("\n");

  const event = envelope("pengaduan.created", pengaduan.id, text);
  if (mediaUrl) event.telegram.photo = mediaUrl;
  return send(event);
}
