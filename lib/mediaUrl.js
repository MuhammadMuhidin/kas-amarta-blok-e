const MEDIA_KEY_PREFIXES = ["activity-images/", "cashflow-receipts/", "payment-proofs/"];
...
export function withMediaPaymentProofUrl(item = {}) {
  if (!item || typeof item !== "object") return item;

  return {
    ...item,
    proof_url: buildMediaUrlFromKey(item.proof_key) || buildMediaUrlFromLegacyUrl(item.proof_url) || item.proof_url || "",
  };
}
