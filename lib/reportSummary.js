...
import { listPublicPaymentProofs } from "@/features/paymentProof/paymentProofRepository";
...
export async function getReportSummary() {
  const [personalRes, paymentRes, cashflowRes, paymentConfirmations] = await Promise.all([
    supabase
      .from(PERSONAL_TABLE)
    ...
    listPublicPaymentProofs(supabase).catch(() => []),
  ]);
