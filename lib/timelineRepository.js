...
export async function getTimelineImageById(id) {
  const supabase = getSupabaseServerClient();
  ...
  if (!data) {
    throw new Error("Foto kegiatan tidak ditemukan");
  }

  return normalizeImage(data);
}

export async function updateTimelineImage(id, payload) {
  const supabase = getSupabaseServerClient();
  ...
  if (Object.keys(safePayload).length === 0) {
    return getTimelineImageById(id);
  }
  ...
  if (error) {
    throw new Error(error.message || "Gagal memperbarui foto kegiatan");
  }
...
export async function deleteTimelineImage(id) {
  const supabase = getSupabaseServerClient();
  ...
  if (error) {
    throw new Error(error.message || "Gagal menghapus foto kegiatan");
  }
...
export async function setTimelinePostReaction(postId, visitorId, reactionType) {
  ...
  if (existingReaction?.id) {
    return getPostReactionResult(supabase, postId, existingReaction.reaction_type);
  }
...
export async function likeTimelinePost(postId, visitorId) {
  ...
  return {
    ...
    reaction_total: result.reaction_total,
  };
}
