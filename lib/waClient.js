...
export async function getWaRuntimeConfig() {
  const [apiUrlValue, sessionIdValue] = await Promise.all([
    getIntegrationConfigString("WA_API_URL"),
    getIntegrationConfigString("WA_SESSION_ID", "main"),
  ]);

  const apiUrl = clean(apiUrlValue).replace(/\/$/, "");
  const apiKey = clean(process.env.WA_API_KEY);
  const sessionId = clean(sessionIdValue) || "main";
...
export async function openWaMessageStream({
  ...
  const { apiUrl, apiKey, sessionId } = await getWaRuntimeConfig();
